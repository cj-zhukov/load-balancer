...
impl LoadBalancer {
    ...
    pub fn forward_request(&mut self, req: Request<Body>) -> ResponseFuture {
        ...
        let mut new_req = Request::builder()
            ...
            .uri(new_uri)
            .body(req.into_body())
            .expect("failed creating request builder");
...
pub async fn handle(req: Request<Body>, load_balancer: Arc<RwLock<LoadBalancer>>) -> Result<Response<Body>, hyper::Error> {
...
async fn main() {
    let worker_hosts = vec![
        "http://localhost:53674".to_string(),
        "http://localhost:53693".to_string(),
    ];
